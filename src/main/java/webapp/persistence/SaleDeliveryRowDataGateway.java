...
public class SaleDeliveryRowDataGateway {
	...
	private static final String INSERT_SALEDELIVERY_SQL = "insert into saledelivery (id, sale_id, customer_vat, address_id) "
			+ "values (DEFAULT, ?, ?, ?)";
	...
	private static final String GET_SALEDELIVERY_BY_ID_SQL = "select id, sale_id, customer_vat, address_id "
			+ "from saledelivery " + "where id = ?";
	...
	public SaleDeliveryRowDataGateway(ResultSet rs) throws RecordNotFoundException {
		try {
			fillAttributes(rs.getInt("sale_id"), rs.getInt("customer_vat"));
	...
	private static final String GET_SALEDELIVERY_BY_CUSTOMER_VAT_SQL = "select * " + "from saledelivery "
			+ "where customer_vat = ?";
	...
	private static SaleDeliveryRowDataGateway loadSaleDelivery(ResultSet rs) throws RecordNotFoundException {
		try {
			SaleDeliveryRowDataGateway newSaleDelivery = new SaleDeliveryRowDataGateway(rs.getInt("sale_id"),
					rs.getInt("customer_vat"), rs.getInt("address_id"));
	...
	private static final String DELETE_SALE_DELIVERY_SQL = "delete from saledelivery " + "where id = ?";
