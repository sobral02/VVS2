...
public enum SaleService {
	...
	public void addSale(int customerVat) throws ApplicationException {
		if (!isValidVAT(customerVat)) // verifica se o VAT é valido
			throw new ApplicationException("Invalid VAT number: " + customerVat);
		else
			try {
				try {
					CustomerService.INSTANCE.getCustomerByVat(customerVat); // tenta obter o cliente com o VAT
				} catch (Exception e) {
					throw new ApplicationException("Invalid VAT number: " + customerVat); // se não conseguir ou seja se
																							// o
																							// cliente não existir esta
																							// exceção é levantada
				}
				SaleRowDataGateway sale = new SaleRowDataGateway(customerVat, new Date()); // se o VAT e o cliente
																							// existirem e foram válidos
				sale.insert(); // adiciona a Sale
	...
	public void removeDeliverySalesByCustomerId(int customerId) throws ApplicationException {
		try {
			List<SaleDeliveryRowDataGateway> deliverySales = new SaleDeliveryRowDataGateway()
					.getAllSaleDelivery(customerId);
			for (SaleDeliveryRowDataGateway deliverySale : deliverySales) {
				deliverySale.deleteSaleDelivery();
			}
		} catch (PersistenceException e) {
			throw new ApplicationException("Error removing delivery sales for customer with VAT number " + customerId,
					e);
		}
	}
